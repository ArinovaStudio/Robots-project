...
export default function UserPostsFeed({ targetUserId }: { targetUserId: string }) {
  ...
  const fetchFeed = useCallback(async (pageNum: number, append = false) => {
    ...
    try {
      const res = await fetch(`/api/user/feed/explore?page=${pageNum}&limit=15&userId=${targetUserId}`);
