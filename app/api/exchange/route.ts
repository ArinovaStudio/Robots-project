...
export async function GET(req: NextRequest) {
  try {
    ...
    const currentCompanyQuery = await prisma.$queryRaw<any[]>`
      ...
      LIMIT 1
    `;
    console.log("This is data fetch from database")
    console.log(currentCompanyQuery);

    if (!currentCompanyQuery || currentCompanyQuery.length === 0) {
      return NextResponse.json(
        { success: false, message: "Profile incomplete." },
        { status: 400 },
      );
    }
    ...
    const crossConnections = getCrossConnections(
      currentCompany.exchangeDataset,
    );
    ...
    formattedData = formattedData.sort(
      (a, b) => (b.isBoosted === true ? 1 : 0) - (a.isBoosted === true ? 1 : 0),
    );
...
//     console.log(user);
//     const currentCompanyQuery = await prisma.$queryRaw<any[]>`
//       SELECT "type", "dealIn", "exchangeDataset", "needsVector"::text as "needsVector", "offeringVector"::text as "offeringVector"
//       FROM "CompanyProfile" 
//       WHERE "userId" = ${user.id}
//       LIMIT 1
//     `;
...
//     let formattedVector = currentCompany.needsVector;
//     // console.log(formattedVector,"this is data",currentCompany);
...
//     if (!formattedVector && currentCompany.offeringVector) {
//       formattedVector = currentCompany.offeringVector;
//     }
